...
export function createRoundedCubeGeometry({
	...
	const capGeometry = new THREE.ShapeGeometry(shape, 10).toNonIndexed()
	capGeometry.rotateX(Math.PI / 2);
	capGeometry.translate(-width / 2, -depth / 2, -height / 2);
	const topCap = capGeometry.clone().translate(0, 0, 0);
	const bottomCap = capGeometry.clone().translate(0, -depth, 0);

	geometry.computeBoundingBox();
	topCap.computeBoundingBox();
	bottomCap.computeBoundingBox();
...
document.body.addEventListener("pointermove", (event) => {
	...
	Object.assign(
		scene.rotation,
		{
			y: -(event.clientX / window.innerWidth) * Math.PI / 12,
			x: (event.clientY / window.innerHeight) * Math.PI / 12 - Math.PI / 15,
...
const deviceMotion = e => {
	...
	// devicemotion.y = containedBetween(-Math.PI / 4, Math.PI / 4)(devicemotion.y + (-beta) / 360 * 0.1 * Math.PI / 4);

	devicemotion.x = containedBetween(-Math.PI / 4.75, Math.PI / 4)(devicemotion.x + (-alpha) / 360 * 0.1 * Math.PI / 4);

	devicemotion.y += (-beta) / 360 * 0.1 * Math.PI / 4

	GSAP.to(
		scene.rotation,
		{ ...devicemotion, duration: 0.3 }
	)
}
